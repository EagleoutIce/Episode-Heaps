...
\long\def\hsStep#1#2#3#4#5{%
\begin{center}
\savebox0{\heap{#1}}\parbox[\ht0+\dp0]{16em}{\centering\downsize\linewidth{\usebox0}}\hfill\begin{tikzpicture}
\foreach[count=\i,remember=\i as \li (initially 0)] \l in {#2} {
    \xdef\curmax{\i}
    \ifnum\li=0
        \node[lblock,minimum size=2em,fill=white,very thick,right,outer sep=0pt] (1) at (0,0) {\l};
    \else
        \node[lblock,minimum size=2em,fill=white,right,outer sep=0pt,very thick] (\i) at (\li.east) {\l};
    \fi
}
...
\ifx!#4!\else
    \draw[gray,Kite-Kite] (d#4.south) to[bend right] (d#5.south);
\fi
